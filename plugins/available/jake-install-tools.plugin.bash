...
function jake-install-tools() {
	...
	_jake-find-tool  btop btop "htop, but overwhelming and pretty"
	...
	local GIT_VERSION="$(git --version)"
	local EXPECTED_VERSION='git version 2.53.0'
	...
	# TODO: I should also check that these ppa declarations *are even there*, and I'm not just lagging
	...
	if grep -s -q "$(whoami)" /etc/wsl.conf &>/dev/null; then
		echo "Nothing to do for setting WSL's default user"
	else
		echo "you're probably not the default wsl user."
...
function _jake-update-ack-and-its-manpages {
	...
	local ack_url="${1:-https://beyondgrep.com/ack-v3.9.0}"
