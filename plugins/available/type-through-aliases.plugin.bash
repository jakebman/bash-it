...
function jake_type {
	...
	case $(command type -t -- "$1") in
		alias)
			...
			# NB: we check .Stmt.Cmd because `time foo` is a TimeClause which contains a CallExpr
			next=$(shfmt <<< "${BASH_ALIASES[${1}]}" --to-json | jq --raw-output '.Stmts[0].Cmd|(.Stmt.Cmd // .).Args[0].Parts[0].Value')
